...
DBG("1: enable_count=%d", mdp5_kms->enable_count);
	pm_runtime_get_sync(dev);
DBG("2: enable_count=%d", mdp5_kms->enable_count);
...
static int mdp5_init(struct platform_device *pdev, struct drm_device *dev)
{
	...
	// TODO should this be before or after pm_runtime_set_active()??
	...
	if (__clk_is_enabled(mdp5_kms->core_clk)) {
		...
		// XXX need to do this for dsi/hdmi/etc depending on what is on..

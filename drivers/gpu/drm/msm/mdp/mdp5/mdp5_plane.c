...
// TODO could possibly share a helper w/ msm_fbdev...  also make msm_fbdev
// re-use this fb if sizes/etc match..
#include "msm_gem.h"
static struct drm_framebuffer *
alloc_stolen_fb(struct drm_device *dev, int w, int h, int p, uint32_t format)
{
...
void mdp5_plane_readback(struct drm_plane *plane, enum mdp5_pipe pipe)
{
	...
	state->fb = alloc_stolen_fb(plane->dev,
			state->src_w >> 16,
			state->src_h >> 16,
			pitch, format);
}

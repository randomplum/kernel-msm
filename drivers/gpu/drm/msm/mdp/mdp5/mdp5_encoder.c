...
static int encoder_readback_setup_crtc_state(struct drm_encoder *encoder)
{

	/* Since mdp5_crtc is fully virtualized, we can pick any (but
	 * then we need to figure out which mixer is used):
	 */
...
void mdp5_encoder_readback(struct drm_encoder *encoder)
{
	...
	if (encoder_readback_setup_crtc_state(encoder))
		return;
